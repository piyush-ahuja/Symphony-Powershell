...
$Names = import-csv "$UserNamesFile" -header Username,newUsername 
...
# Look up ID from Username
  try {
        if ($proxy) {    $fullUserList = Invoke-RestMethod -Method GET -Headers $hdrs  -Proxy $proxy -Uri "$podUrl/pod/v2/user?username=$Username" }
        else        {    $fullUserList = Invoke-RestMethod -Method GET -Headers $hdrs -Uri "$podUrl/pod/v2/user?username=$username" }
...
$DataList = @{}
$DataList.Add("userName",$newUsername)
